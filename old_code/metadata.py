...
def ptnm_tn_class_to_one_hot(
    my_class: Union[str, float],
    ptnm_labels_hierarchy: Dict[str, List[str]],
...
def ptnm_tn_one_hot_to_class(
    ...
    indexes_of_big_classes = [
        valid_ptnm_labels.index(s) for s in ptnm_labels_hierarchy.keys()
    ]
    # indexes_of_small_classes = {k: [valid_ptnm_labels.index(vv)] for k, v in ptnm_labels_hierarchy.items() for vv in v}
    indexes_of_small_classes = [
        valid_ptnm_labels.index(vv) for v in ptnm_labels_hierarchy.values() for vv in v
    ]
    ...
    if all([t[i] < epsilon for i in small_class_i]):
        ...
        if small_class_label in ptnm_labels_hierarchy[big_class_label]:
            ...
            print(
                f"warning: possible ambiguous predictions between {big_class_label} and {small_class_label}"
            )
