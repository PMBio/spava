...
from old_code.data2 import quantiles_for_normalization, file_path
...
class VAE(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        x, mask, corrupted_entries = batch
        # x = batch
...
def get_loaders(
    ...
    train_ds = FakeRGBCells("train")
    train_ds_validation = FakeRGBCells("train")
    val_ds = FakeRGBCells("validation")
...
def objective(trial: optuna.trial.Trial) -> float:
    ...
    trainer = pl.Trainer(
        ...
        log_every_n_steps=8 if not ppp.DEBUG else 1,
        val_check_interval=1 if ppp.DEBUG else 8,
    )
