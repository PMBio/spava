# adapted from pl_bolts.models.autoencoders.components
...
class ResNetDecoder(nn.Module):
    ...
    def __init__(self, block, layers, latent_dim, input_height, first_conv, maxpool1, n_channels):
        ...
        if self.maxpool1:
            self.layer3a = self._make_layer(block, 64, layers[2], scale=2)
            self.layer3b = self._make_layer(block, 64, layers[2], scale=2)
            self.upscale_factor *= 2
        else:
            self.layer3a = self._make_layer(block, 64, layers[2])
            self.layer3b = self._make_layer(block, 64, layers[2])
